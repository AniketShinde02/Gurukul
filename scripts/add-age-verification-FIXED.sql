...
DROP POLICY IF EXISTS "Users can view their own verification logs" ON age_verification_logs;
CREATE POLICY "Users can view their own verification logs"
ON age_verification_logs FOR SELECT
USING (auth.uid() = user_id);

-- ✅ FIX: Allow trigger/system to insert verification logs
-- This policy allows inserts when the user_id matches the authenticated user
DROP POLICY IF EXISTS "System can insert verification logs" ON age_verification_logs;
CREATE POLICY "System can insert verification logs"
ON age_verification_logs FOR INSERT
...
BEGIN
    ...
    RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER; -- ✅ SECURITY DEFINER allows trigger to bypass RLS
